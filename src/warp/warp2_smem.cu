...
#define THREADS_PER_GROUP 16  // WARP_SIZE / COLS_PER_WARP

__global__ void warp2SmemKernel(const half *__restrict__ A, const half *__restrict__ B, half *__restrict__ C, size_t N,
                                size_t K) {
...
size_t initWarp2Smem(size_t K) {
    ...
    HGEMV_CHECK_CUDART_ERROR(
        cudaFuncSetAttribute(warp2SmemKernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_max_size));

    return smem_max_size;
}

void warp2Smem(half *A, half *B, half *C, size_t N, size_t K) {
    static size_t smem_max_size = initWarp2Smem(K);

    dim3 block(THREADS_PER_BLOCK);
    dim3 grid(div_ceil(N, COLS_PER_BLOCK));

    warp2SmemKernel<<<grid, block, smem_max_size>>>(A, B, C, N, K);
}
